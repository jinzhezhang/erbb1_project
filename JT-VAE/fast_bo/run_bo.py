...
def main_bo(vocab_path,
    ...
    # Path of the files
    latent_feature = os.path.join('fast_bo/descriptors/erbb1_latent_features.txt')
    ...
    erbb1_score = os.path.join('fast_bo/descriptors/erbb1_scores.txt')
    # We load the data (y is minued!)
    X = np.loadtxt(latent_feature)
    #y = -np.loadtxt(target)
    y = np.loadtxt(erbb1_score)
    y = y.reshape((-1, 1))
    ...
    while iteration < iterations:
        ...
        for i in range(len(valid_smiles)):
            ...
            if mol!=None:
                ...
                1, 'erbb1_clean_log_ic50', config.device, [valid_smiles[i]], dicts)
