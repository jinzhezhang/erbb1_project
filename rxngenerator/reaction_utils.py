...
def get_template_order(rxn):
	...
	for depth in order.keys():
		n_tems += len(order[depth])
	print(order)
	if n_tems != len(tem_nodes):
		print(len(rxn.route))
		print(n_tems, len(tem_nodes))
		print(visisted)
		print(rxn.route)
		for template_node in tem_nodes:
			print(template_node.id, template_node.template)
		for molecule_node in mol_nodes:
			print(molecule_node.id, molecule_node.smiles)
		print("DKM")
