...
class FTEncoder(nn.Module):
	...
	def __init__(self, ftvocab, hidden_size, embedding=None):
		...
		self.W_r = nn.Linear(hidden_size, hidden_size, bias=False).to(device)
		self.U_r = nn.Linear(hidden_size, hidden_size).to(device)
		self.W_h = nn.Linear(2 * hidden_size, hidden_size).to(device)
		self.W = nn.Linear(2* hidden_size, hidden_size).to(device)
...
def node_aggregate(nodes, h, embedding, W):
	...
	return nn.ReLU()(W(node_vec)).to(device)
