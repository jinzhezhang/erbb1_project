...
class FTRXNVAE(nn.Module):
	def __init__(self, fragment_vocab, reactant_vocab, template_vocab, hidden_size, latent_size, depth, fragment_embedding=None, reactant_embedding=None, template_embedding=None):
		...
		if fragment_embedding is None:
			self.fragment_embedding = nn.Embedding(self.fragment_vocab.size(), hidden_size).to(device)
		else:
			self.fragment_embedding = fragment_embedding

		if reactant_embedding is None:
			self.reactant_embedding = nn.Embedding(self.reactant_vocab.size(), hidden_size).to(device)
		else:
			self.reactant_embedding = reactant_embedding

		if template_embedding is None:
			self.template_embedding = nn.Embedding(self.template_vocab.size(), hidden_size).to(device)
		else:
		...
		self.combine_layer = nn.Linear(2 *hidden_size, hidden_size).to(device)

		self.FT_mean = nn.Linear(hidden_size, int(latent_size)).to(device)
		self.FT_var = nn.Linear(hidden_size, int(latent_size)).to(device)

		self.RXN_mean = nn.Linear(hidden_size, int(latent_size)).to(device)
		self.RXN_var = nn.Linear(hidden_size, int(latent_size)).to(device)
