...
# res = predict_vec(gnn_model,'regression','../../gnn/dataset/regression/erbb1_clean_log_ic50/', 1, 'erbb1_clean_log_ic50', config.device, ['NCc1cccc(-c2cc3ncnc(Nc4ccc5[nH]ncc5c4)c3s2)c1'], dicts)
# print(res)
...
def run_bo(X_train, y_train, X_test, y_test, model, parameters, metric, randseed):
	...
	filename = "Results/"+ metric + str(random_seed) + "_neg.txt"
	...
	with open(filename, "w") as writer:
		...
		while iteration < 30:
			...
			next_inputs, values = sgp.batched_greedy_ei(10, np.min(X_train, 0), np.max(X_train, 0))
			valid_smiles =[]
			new_features =[]
			full_rxn_strs=[]
			...
			for i in range(10):
				...
				res= decode_many_times(model, torch.from_numpy(latent).float().to(device))
				if res is not None:
			...
			for i in range(len(valid_smiles)):
				...
				if metric=="erbb1":
					mol = rdkit.Chem.MolFromSmiles(valid_smiles[i])
					if mol!=None:
						res = predict_vec(gnn_model,'regression','../../gnn/dataset/regression/erbb1_clean_log_ic50/', 1, 'erbb1_clean_log_ic50', config.device, [valid_smiles[i]], dicts)

						print(valid_smiles[i], '|', res[0])
						score = res[0]
						scores.append(score)
						b_valid_smiles.append(valid_smiles[i])
						b_full_rxn_strs.append(full_rxn_strs[i])
...
if os.path.isfile('fragmentDic.pkl') and os.path.isfile('datapairs.pkl'):
    fragmentDic = pickle.load(open('fragmentDic.pkl','rb'))
    data_pairs = pickle.load(open('datapairs.pkl','rb'))
else:
    n_pairs = len(routes)
    ...
    pickle.dump(fragmentDic, open('fragmentDic.pkl','wb'))
    pickle.dump(data_pairs, open('datapairs.pkl','wb'))
...
#python run_bo.py -w 200 -l 50 -d 2 -r 2 -v "weights/data.txt_fragmentvocab.txt" -t "data.txt" -s "weights/uspto_vae_iter-100.npy" -m "erbb1"
